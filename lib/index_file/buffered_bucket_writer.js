...
class BufferedBucketWriter {
  ...
  search(index, hash, value) {
    ...
    if (offset < 0) return this._compareFileRecord(position, hash, value);
    ...
    return this.search(ptr32[offset32], hash, value);
  }

  _compareFileRecord(position, hash, value) {
    ...
    .then(() => {
      ...
      if (ptr32[1] === hash) {
        ...
        .then(() => {
          if (buffer8.toString('utf8', 0, length) === value) {
            return true;
          }
          else return this.search(nextIndex, hash, value);
        })
      }
      else return this.search(nextIndex, hash, value);
    });
