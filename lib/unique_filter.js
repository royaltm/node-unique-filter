...
class UniqueFilter {
  ...
  static getOptimalOptions(iterations = 1000000, probability = 0.1) {
    ...
    iterations >>>= 0;
    var m = -iterations * Math.log(probability) / Math.pow(Math.LN2, 2);
  ...
  byteUsage() {
    var bloom = this.bloom
      , index = this.index
    return index.ready.then(bbw => {
      var lruSet = bbw.lruSet;
      return (lruSet ? 2 * lruSet.length : 0) +
             byteLength(bloom.buckets) +
             byteLength(bloom._locations) +
             byteLength(index.index) +
             byteLength(bbw.writeBuffer) +
             byteLength(bbw.readBuffer);
    })
  }
